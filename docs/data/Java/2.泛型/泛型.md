...
13、如果有两个方法，一个是普通方法 `show(String s)`，一个是泛型方法 `<T> show(T t)`，当传入 String 参数时会调用哪个？

- **结论**：代码可以正常编译和运行，编译器会优先调用**普通方法** `show(String s)`。

- **原理**：这是 Java 的**方法重载（Overloading）**机制。当有多个方法都能匹配传入的参数时（泛型方法可以匹配任意类型，普通方法匹配特定类型），Java 编译器遵循**“最具体匹配原则” (Most Specific Match)**。
  - 普通方法 `show(String s)` 对于 String 类型的数据来说，比泛型方法 `<T> show(T t)` 更具体、更明确。
  - 因此，编译器会“抢先”锁定普通方法。只有当传入的参数无法匹配普通方法时（例如传入 `Integer`），才会去调用泛型方法。
...
public class OverloadTest {
    ...
    public static void main(String[] args) {
        ...
        test.show("Hello"); // 输出：普通方法
        test.show(123);     // 输出：泛型方法
        test.<String>show("Hello"); // 输出：泛型方法（显式指定，强制调用）
    }
}