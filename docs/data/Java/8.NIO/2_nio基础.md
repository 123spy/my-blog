...
1. 发出指令：CPU 向磁盘控制器发出读取指令，要求读取某个扇区的数据。

2. 轮询等待 (Polling)：由于磁盘太慢，CPU 必须不断地询问磁盘：“好了吗？好了吗？”。在这个过程中，CPU 无法执行其他计算任务，处于忙等状态。

3. 逐字节搬运：磁盘数据准备好后，放在磁盘控制器的缓冲区（通常只有几个字节）。

4. CPU 亲自劳动：CPU 必须执行一条指令，从硬件寄存器里读出 1 个字节，再执行一条指令，把它存入内存。

5. 循环往复：如果要读 1024 字节，CPU 就要手动循环搬运 1024 次。



结论：

在没有 DMA 的年代，IO 操作会占用 100% 的 CPU。即便数据只是在路途上，CPU 也得在那儿守着，极大地浪费了算力。
...
Q：假设此时需要读取一个1024B的文件，在使用和不使用buffer的区别又是什么那？

- 未使用buffer：
  - 普通不使用buffer的BIO操作，每一次操作的单位是1字节，也就是说会发生1024 * 4次的状态切换。这是一个巨大的性能问题。

- 使用buffer：
  - 使用了buffer，对于步骤1，3，4可以直接一次性拷贝，由原来1个字节1个字节拷贝共1024次操作，变成了一次性拷贝1024个字节的1次操作。性能提升了原来的1/1024倍。



## 操作系统DMA优化

在步骤二的时候，内核通过DMA从磁盘搬运时，通常会以 “页” (Page) 为单位（通常是 4KB）。

也就是说，无论如何并不是1个字节1个字节的从磁盘读取到内核空间。而是一次性将4KB的数据读取到内核空间。因此步骤二只运行了1次。
